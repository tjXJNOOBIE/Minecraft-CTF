...
async function ensureMultiviewReady(): Promise<void> {
  ...
  console.log(`[stable] Multiview ready: ${multiviewUrl}`);
  console.log(`[stable] Registry ready: ${registryUrl}`);
}
...
async function maybeScaleViewTunnels(input: any): Promise<void> {
  ...
  viewCount = desired;
  console.log(`[stable] Scaling view tunnels to ${viewCount} views`);
...
async function startViewTunnelProcess(): Promise<void> {
  if (!enableViewTunnels) {
    console.log("[stable] View tunnels disabled by CTF_ENABLE_VIEW_TUNNELS=0");
    return;
  }
  if (tunnelProcess) {
    return;
  ...
  tunnelProcess.on("error", (err) => {
    console.error("[stable] View tunnel failed:", err.message);
  });
  tunnelProcess.on("exit", (code) => {
    if (code !== null && code !== 0) {
      console.error("[stable] View tunnel exited with code", code);
    }
  });
  ...
  console.log(`[stable] View tunnels local:${viewBasePort}-${viewBasePort + viewCount - 1} -> remote:${viewBasePort}-${viewBasePort + viewCount - 1}`);
}
  ...
  viewHealthTimer = setInterval(async () => {
    ...
    for (const url of checks) {
      ...
      if (prev === undefined || prev !== ok) {
        ...
        console.log(`[stable] camera route ${ok ? "UP" : "DOWN"} ${url}`);
      }
    }
  }, 2000);
}
...
async function ensureRemoteViewerDependencies(): Promise<void> {
  ...
  try {
    ...
    console.log("[stable] Installing remote viewer dependencies (prismarine-viewer, canvas)...");
  }
...
async function main(): Promise<void> {
  ...
  await startViewTunnelProcess();
  startViewHealthLoop();
  console.log("[stable] Starting REMOTE headless CTF bot simulation");
  ...
  processRef.on("error", (err) => {
    ...
    stopViewTunnelProcess();
    console.error("[stable] Failed to launch remote bot script", err.message);
    process.exit(1);
  });
  processRef.on("exit", async (code) => {
    ...
    stopViewTunnelProcess();
    await syncRemoteRegistryToLocal();
    console.log("[stable] Remote bot script finished with code", code);
    process.exit(code ?? 0);
  });
}
